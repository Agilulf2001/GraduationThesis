...
public class Util {
    ...
    public static List<Integer> getRandomList(int total, int needed, double skewRatio)
    {
        skewRatio = 0.0;
        ...
        random.setSeed(seed);
        int num = 0;
        int elephant = (int)(skewRatio*needed);   // 象流数
        // System.out.printf("ELE IIIIS:%d\n",elephant);
        // elephant=300;
        // while (num < needed)
        // {
        ...
        // }
        while (num < elephant) {
            int tmp = Math.abs(random.nextInt()) % elephant;
            while (set.contains(tmp)) {
                tmp = Math.abs(random.nextInt()) % elephant;
            }
            set.add(tmp);
            num++;
        }
        while (num < needed) {
            int tmp = elephant + (Math.abs(random.nextInt()) % (total-elephant));
            while (set.contains(tmp)) {
                tmp = elephant + (Math.abs(random.nextInt()) % (total-elephant));
            }
            set.add(tmp);
            num++;
        }
