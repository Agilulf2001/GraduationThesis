...
public class Packet_Generator {
    ...
    public Packet_Generator(int INT_PPS,int persist_time,ArrayList<FlowInfo> datas)
    {
        ...
        for(FlowInfo info:this.datas)
        {
            ...
            // if (info.realSendNum != 0) {
            //     System.out.println("SEND AGAIN!");
            // }
            info.realSendNum = 0;
            int packetLim = info.packetNum;
            // 未考虑info.packetNum参数，模拟所有流在查询时活跃
            while(current_time < this.TotalTimeUnit && realSendNum < packetLim)
            {
                packet_sent.get(current_time).add(info);
                realSendNum++;
                current_time += time_interval;
                // if (realSendNum >= packetLim) { // 03.25 add
                //     break;
                // }
            }
    ...
    public ArrayList<FlowInfo> GetInfos()     //return all the packets in current ms 
    {
        if(current_time>=TotalTimeUnit)
            return null;
        // System.out.printf("current_time: %d\n", current_time);
